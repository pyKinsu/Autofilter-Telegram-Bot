...
async def filter_languages_cb_handler(client: Client, query: CallbackQuery):
    ...
    if offset != "":
        try:
            ...
            await save_group_settings(query.message.chat.id, 'max_btn', True)
            btn.append(
                [InlineKeyboardButton("Page", callback_data="pages"), InlineKeyboardButton(text=f"1/{math.ceil(int(total_results)/10)}",callback_data="pages"), InlineKeyboardButton(text="Nᴇxᴛ ➪",callback_data=f"next_{req}_{key}_{offset}")]
            )
    else:
        btn.append(
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
                        ...
                        await client.send_cached_media(
                            ...
                            reply_markup=InlineKeyboardMarkup(
                                ...
                                [
                                    ...
                                    InlineKeyboardButton('Uᴘᴅᴀᴛᴇs Cʜᴀɴɴᴇʟ ⇀', url="t.me/sanyamovies")
                                ],[
                                    InlineKeyboardButton(" ᴍᴀɪɴ ᴄʜᴀɴɴᴇʟ ", url="t.me/kissubots")
                                ]
                                ]
                            )
                        )
        ...
        btn2 = [[
                 ...
                 InlineKeyboardButton("", url="t.me/kissu_help_bot")
               ]]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
            reply_markup = InlineKeyboardMarkup(btn)
        ...
        buttons = [[
            InlineKeyboardButton('✉️ ᴀɴʏ ǫᴜᴇʀʏ ✉️', url='t.me/kissuhelp')
        ], [
        ...
        buttons = [[
            InlineKeyboardButton('ꜱᴏᴜʀᴄᴇ ᴄᴏᴅᴇ', url="https://github.com/pyKinsu/autofiltertelegrambot")
            ],[
    ...
    [InlineKeyboardButton('‹‹‹ Bᴀᴄᴋ', callback_data='about')]
        ...
        await query.edit_message_media(InputMediaPhoto(random.choice(PICS), script.STATUS_TXT.format(total, users, chats, monsize, free), enums.ParseMode.HTML), reply_markup=InlineKeyboardMarkup(buttons))
            ...
            btn = [[
                    ...
                    InlineKeyboardButton("↼ ʙᴏᴛ ᴄʜᴀɴɴᴇʟ ⇀", url="t.me/kissubots")
                ],[
                    InlineKeyboardButton("↼  Uᴘᴅᴀᴛᴇ Cʜᴀɴɴᴇʟ ⇀", url="t.me/sanyamovies")
                ],[
                    InlineKeyboardButton('🛖 Hᴏᴍᴇ', callback_data='start'),
                    InlineKeyboardButton('Cʟᴏsᴇ ✘', callback_data='close_data')
                ]]
    ...
    if not g_s:
        ...
        button = [[
                   InlineKeyboardButton("🔍 Tap To Check Spelling ", url=f"https://www.google.com/search?q={reqst_gle}")
                    ...
                    InlineKeyboardButton("New Released Movies Here Join", url=f"https://t.me/sanyamovies")
        ]]
        if NO_RESULTS_MSG:
